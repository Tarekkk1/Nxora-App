...
class FirebaseService {
  ...
  Future<void> createStudent({
    ...
    final String studentId = getUID('users');
    final Map<String, dynamic> data = {
      ...
      'platform': platform,
      'disabled': isDisabled,
      'role': [], // Empty role array for regular students
      'enrolled': [], // Empty enrolled courses array
      'wishlist': [], // Empty wishlist
      'completed_lessons': [], // Empty completed lessons
      'created_at': FieldValue.serverTimestamp(),
      'updated_at': FieldValue.serverTimestamp(),
    };
    
    await firestore.collection('users').doc(studentId).set(data);
  }
  ...
  Future<List<Course>> getAllPublishedCourses() async {
    ...
    await firestore
        .collection('courses')
        .where('status', isEqualTo: 'live') // Only get published/live courses
        .orderBy('created_at', descending: true)
        .get()
  ...
  Future<void> updateStudentCourses({
    ...
    for (String courseId in coursesToAdd) {
      batch.update(userRef, {
        'enrolled': FieldValue.arrayUnion([courseId])
      });
    }
    
    // Remove courses from student's enrolled list
    for (String courseId in coursesToRemove) {
      batch.update(userRef, {
        'enrolled': FieldValue.arrayRemove([courseId])
      });
    }
